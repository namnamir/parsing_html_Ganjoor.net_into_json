...
file = '/home/namnam/Downloads/Iransec/00-source/Scrapy/poem.json'
...
def close_file(poems):
	global file
	# using codecs helps to fix the problem of unicode. It write as UTF8 into the json file instead of writing as unicode or unicode-escape | read more: http://code.opoki.com/loading-utf-8-json-file-in-python/
...
def parse_links(link, link_style, poet, book):
	j = 0
	# opening the json file
	poem_list = open_file()
...
for i in range(7, len(poets)):
	if i == 0:
		...
		poet = 'خیام'
		book = 'رباعیات خیام'
	parse_links(poets[i], style[i], poet, book)
